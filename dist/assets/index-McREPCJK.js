...
`,R=document.getElementById("problem-title"),q=document.getElementById("problem-instruction"),N=document.getElementById("progress-label"),I=document.getElementById("prev-btn"),g=document.getElementById("next-btn"),y=document.getElementById("check-btn"),h=document.getElementById("result-area"),S=document.getElementById("preview-frame"),E=document.getElementById("iframe-cover"),k=document.querySelectorAll("#tabs .tab"),U=document.getElementById("monaco-container"),F=document.getElementById("top-pane"),V=document.getElementById("bottom-pane"),H=document.getElementById("editor-pane");document.getElementById("preview-pane");const K=document.getElementById("h-splitter"),W=document.getElementById("v-splitter"),Y=document.getElementById("app");let m=[],d=0,b=[],u=null,r={},w="html",_=null;function G(){return new Promise(e=>{require.config({paths:{vs:"https://cdn.jsdelivr.net/npm/monaco-editor@0.45.0/min/vs"}}),require(["vs/editor/editor.main"],()=>e())})}function J(){r.html=monaco.editor.createModel(O,"html"),r.css=monaco.editor.createModel(z,"css"),r.js=monaco.editor.createModel(D,"javascript"),u=monaco.editor.create(U,{model:r.html,theme:"vs-dark",automaticLayout:!1,fontSize:14,minimap:{enabled:!1}}),Object.values(r).forEach(e=>{e.onDidChangeContent(()=>{X()})}),k.forEach(e=>{e.addEventListener("click",()=>{k.forEach(a=>a.classList.remove("active")),e.classList.add("active"),w=e.dataset.lang,u.setModel(r[w])})}),B()}function X(){clearTimeout(_),_=setTimeout(B,300)}function B(){const e=r.html.getValue(),a=r.css.getValue(),l=r.js.getValue(),f=`<!DOCTYPE html>
<html lang="ja">
<head>
<meta charset="UTF-8" />
<style>${a}</style>
...
</html>`;S.srcdoc=f}async function Q(){const e=[];for(const a of A){const l=await $(()=>import(`/problems/${a}`),[]);e.push(l.default)}return e}function x(){const e=m[d];R.textContent=e.title,q.textContent=e.instruction,N.textContent=`課題 ${d+1} / ${m.length}`,I.disabled=d===0,g.disabled=d===m.length-1||!b[d],Z()}function L(e,a){h.textContent=a,h.classList.add("show"),h.classList.toggle("success",e),h.classList.toggle("fail",!e)}function Z(){h.classList.remove("show","success","fail"),h.textContent=""}async function ee(){const e=m[d];y.disabled=!0,y.textContent="判定中...";try{const a=await e.check(S);L(a.success,a.message),a.success&&(b[d]=!0,g.disabled=d===m.length-1)}catch(a){L(!1,`チェック実行中にエラーが発生しました: ${a.message}`)}finally{y.disabled=!1,y.textContent="チェック"}}function te(){d!==0&&(d-=1,x())}function ne(){d===m.length-1||!b[d]||(d+=1,x())}function oe(){let e=null;function a(n){return t=>{e=n,E.classList.add("active"),t.preventDefault()}}function l(n){if(e){if(e==="h"){const t=Y.getBoundingClientRect();let o=n.clientY-t.top;const i=60,c=t.height-100;o=Math.max(i,Math.min(c,o));const s=o/t.height*100;F.style.height=`${s}%`}else if(e==="v"){const t=V.getBoundingClientRect();let o=n.clientX-t.left;const i=150,c=t.width-150;o=Math.max(i,Math.min(c,o));const s=o/t.width*100;H.style.width=`${s}%`}u&&u.layout()}}function f(){e&&(e=null,E.classList.remove("active"),u&&u.layout())}K.addEventListener("mousedown",a("h")),W.addEventListener("mousedown",a("v")),window.addEventListener("mousemove",l),window.addEventListener("mouseup",f),window.addEventListener("resize",()=>{u&&u.layout()})}async function ae(){oe(),I.addEventListener("click",te),g.addEventListener("click",ne),y.addEventListener("click",ee),m=await Q(),b=new Array(m.length).fill(!1),await G(),J(),x()}ae();
